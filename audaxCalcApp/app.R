...
#'  Turn a date object in to a long string e.g. Monthday 11th January 2022
  longDate <- function(date){
    paste(
      ...
      append_date_suffix(date), lubridate::month(date, label = TRUE, abbr = FALSE), 
      year(date))
  }
  ...
  getSunTimes <- function(){
    ...
    observeEvent(input$preset,{
        ...
        if (input$preset == "Cambridge Pork Pie 200"){
            ...
            updateDateInput(session, inputId = "date", value = "2024-03-16")
        }
    })
...
#        theme(legend.position = "none") + 
    ...
    loadActivities <- reactive({
      ...
      activities[,`Avg Total Speed (kph)`:=average_speed %>% as.numeric()]
      activities[,`Avg Moving Speed (kph)`:= distance/(elapsed_time/60/60)]
      activities[,`Move Ratio`:= moving_time/elapsed_time]
      activities[,difficulty:=distance * (total_elevation_gain / 1000)]
      return(activities)
    })
      ...
      req(input$file1)
        ...
        plotFormAndFitness <- function(activities, forcast.length = 30, history.length = 180) {
            tmp <- data.table(
                ...
                stress = activities$suffer_score %>% as.numeric(),
                name=activities$n)
            
            tmp <- tmp[!is.na(stress),]
