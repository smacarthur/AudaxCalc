...
list(
  ...
  "HR (bpm)"            = tmp[, na.omit(average_heartrate %>% as.numeric())] %>% getStats(dp = 0),
  "Power (W)"           = tmp[, na.omit(average_watts %>% as.numeric())]     %>% getStats(dp = 0),
  "Cadence (rpm)"       = tmp[, na.omit(average_cadence %>% as.numeric())]   %>% getStats(dp = 0),
